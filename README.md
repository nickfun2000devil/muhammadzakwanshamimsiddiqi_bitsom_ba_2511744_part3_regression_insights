...
* **Selected Final Model:**
  $$\text{monthly\_sales} = 391,327.13 + 1.1428(M) + 33.7593(F) - 72,697.03(D) + 13,427.78(R_N) + 21,268.10(R_S) + 19,854.55(R_W)$$
...
To review the structural architecture and validation logs of this project, navigate to the local `/screenshots` directory folder to view:
1. `simple_regression_output.png` - Validation card for Model 1 metrics.
2. `multiple_regression_output.png` - Summary card for our final master multi-predictor model.
3. `residuals_preview.png` - Excel sheet layout mapping positive and negative outlier observations.